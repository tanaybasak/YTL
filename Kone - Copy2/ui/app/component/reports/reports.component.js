(function() {
	...
	function ReportsController($scope, HTTPGETService, HTTPPOSTService, $rootScope, $cookies, dialogs, $timeout,_) {
		...
		$scope.driverLicenseNo = $cookies.get('carname');		$scope.mapData = {};		var baseURL = 'https://ytlapiflow.mybluemix.net/db/';
		var requestURL;
...
// MAP STUFF END
		...
		$scope.retrieveTripDetails = function(){
			...
			HTTPGETService.get(requestURL).then(function(response) {
				...
				HTTPGETService.get('https://newytl.mybluemix.net/TRIP').then(function(response1)
				{				

							$scope.data2= DEF(response1.data);
							
				HTTPGETService.get('https://newytl.mybluemix.net/driver/behaviouR').then(function(response2)
		...
		function getMapRoutes() {
			...
			var requestURL = 'https://newytl.mybluemix.net/trip/' + $scope.tripId; //here replace '2376265961' with trip id
			var mapCoordinates = [];
			HTTPGETService.get(requestURL).then(function(response) {
...
function DEF(subDetails){
                var tripIdArr = [];
                var temp = [];
                console.log(subDetails[1].AVG_SPEED);
