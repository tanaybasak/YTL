...
_.extend(driverInsightsAnalyze, {
	TRIP_ANALYSIS_STATUS: {
		...
		RUNNING: {id: "4", status: "RUNNING", message: "Driver behavior results running"},
	},
	...
	sendJobRequest: function(from/*YYYY-MM-DD*/, to/*YYYY-MM-DD*/){
		console.log("-------------analyze::sendJobRequest--------------------");
		//this.getSummaryList('d8812b07-d577-434b-91f1-a75c3f79355c');
		//this.getSummaryList('5f25b300-0600-4a46-9203-e079dc19aa46');
		//return;
	...
	_sendJobRequest: function(from, to){
		console.log("--------------analyze::_sendJobRequest-----------------");
		if(!from || !to){
	...
	*/
			...
			self.watchingJobs.forEach(function(watchedId, index){
				self.getJobInfo(watchedId, function(watchedInfo){
					if(watchedInfo.job_status === "RUNNING"){
						console.log("-------------Current Job Status ",watchedInfo.job_status);
						self._setWatchingTask();
					}else if(watchedInfo.job_status === "SUCCEEDED"){
						//console.log("-------------calling update status ");
						driverInsightsTripRoutes.setJobStatus(watchedId, watchedInfo.job_status);
						//console.log("-------------calling get summary list ");
						self.getSummaryList(watchedId);
						
						
						//commented because dont required to remove job
	...
	getSummaryList: function(job_id){
		console.log("--------------------job_id ", job_id);
		var self = this;
		var deferred = Q.defer();
		var config = this.driverInsightsConfig;
		var url = config.baseURL+'/drbresult/tripSummaryList?tenant_id='+config.tenant_id + '&job_id=' + job_id;
		var options = {
			method: 'GET',
			url: url
		};
		...
		request(options, function(error, response, body){
			...
			if (!error && response.statusCode === 200) {
				var value = JSON.parse(body);
				if(value.length == 0 && job_id){
					console.log("Trip Summary Details not found.");
					deferred.reject({job_id: job_id});
				}else{
					dashdbHelper.insertTrips(value, function(already_processed, e){
						if(!e){
							//var ids = _.pluck(value, "id");
							var ids = [];
							_.each(value, function(trip, index){
								var processed = _.contains(already_processed, trip.trip_id);	
								if(!processed){
									ids.push(trip.id);
								}else{
							...
							if(ids && ids.length > 0){
								...
								async.each(ids, function(id, callback) {
									var trip_uuid = id.trip_uuid;	
									
									self._getDetail(null, trip_uuid).then(function(results){
										//console.log("---------results ", results);
										if(results){
											var tripScore = score.calculateBehaviorScores(results);
											//console.log("tripScoretripScore>>>>>>>>>>>>>>>>>>>>> " ,tripScore);
											results.trip_score = tripScore;
											tripsDetail.push(results);
										}else{
											console.log("Trip details not found for trip_uuid="+trip_uuid);
										}
										callback();
									}, function(error){
										console.error('analyze::getSummaryList::Error:' + error);
										console.log("Error for trip_uuid : ", trip_uuid);
										deferred.reject({message: 'analyze::getSummaryList::Error:' + error});
										callback();
									});	
									...
									if( err ) {
									  console.log('analyze: error(getSummaryList): ' + err);
									} else {
										console.log('processed successfully');
										if(!tripsDetail || tripsDetail.length == 0){
											console.log("Trip details not found for process");
										}else{
											driverInsightsTripRoutes.setTripsDetails(tripsDetail);
										}	
									}
								});
							}			
							...
							var msg = 'analyze: error(getSummaryList): '+ body;
							deferred.reject({message: msg, job_id: job_id});
						}
					});
				}
			} else {
