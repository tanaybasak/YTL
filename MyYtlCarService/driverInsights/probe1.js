...
IOTF.on("+", function(payload, deviceType, deviceId){
	...
	(driverInsightsProbe.mapMatch(deviceType, deviceId, payload).then(function(prob){
		...
		var prob = {
				...
				//Adding following parameters for simulator: By Shraddha
				"time_travelled":payload.time_travelled,
				"current_time":payload.current_time,
				"current_location":payload.current_location,
				"dist_covered":payload.dist_covered
				//end adding*/
		};
		consol.log("time_travelled"+time_travelled);		return prob;
	})).then(function(prob){
...
//			if(!matched.road_type && prob.matched_link_id){
	...
	sendProbeData: function(carProbeData, callback) {
		...
		var api = "/datastore/carProbe";   //api not found
